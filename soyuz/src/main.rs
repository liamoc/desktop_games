...
mod game;
mod drawing;
use game::{Tetris, Status,TickResult};
        ...
        for event in event_pump.poll_iter() {
            ...
            match event {
                ...
                Event::KeyDown { keycode: Some(Keycode::F9), ..} => {
                    if micro_mode {
                        micro_mode = false;
                        canvas.window_mut().set_size(224,289).unwrap_or_default();
                    } else {
                        micro_mode = true;
                        canvas.window_mut().set_size(224/2,289/2).unwrap_or_default();
                    }
                }
...
pub fn main() {
    ...
    canvas.set_logical_size(224,289).unwrap();
